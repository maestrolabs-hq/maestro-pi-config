...
| `pi/extensions/` | `<pi agent dir>/extensions/` | five Pi extensions, including the herdr integration hook |
...
### The herdr Pi extension is version-coupled

`pi/extensions/herdr-agent-state.ts` is generated by `herdr integration
install pi`, not hand-written, and it is captured verbatim -- so it is tied to
the herdr version that generated it on this machine. Restoring it onto a
