...
This copy is adapted for pi. Upstream ships `/loop` and `ultracode`, which are Claude Code features; both are replaced here with plain instructions any agent can follow.

Builders and critics run as parallel subagents through pi's `subagent` tool. The structure carries over unchanged.
